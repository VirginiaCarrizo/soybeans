...
async def predict(file: UploadFile = File(...)):
    ...
    for (x1,y1,x2,y2) in boxes:
        ...
        # 5) Calcular bounding box de la máscara de semilla
        ys, xs = np.where(seed_mask==255)
        if ys.size==0: continue
        # dentro del ROI padded, coordenadas:
        ...
        # 6) Dibujar líneas de medida en composed
        # ancho: horizontal línea
        cv2.line(composed,
                 (gx1, gy1 + (gy2-gy1)//2),
                 (gx2, gy1 + (gy2-gy1)//2),
                 (0,255,0), 2)
    ...
    # 7) (Opcional) Etiquetas de confidence
